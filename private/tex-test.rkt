...
(require redex/reduction-semantics
         ...
         "../../trace-contract/model/surface.rkt"
         "../../trace-contract/model/semantics.rkt")

(define-rewriters surface-rewriters
  #:compound
  'mon (λ (j k l e_κ e)
         (list "\\texttt{mon}"
               (format "_{~a}" (texexpr->string j))
               (format "^{~a,~a}" (texexpr->string k) (texexpr->string l))
               e_κ "\\," e))
  'λ (λ (x e) (list "\\lambda" x "." e))
...
(parameterize ([current-rewriters surface-rewriters])
  (displayln
   (texexpr->string
    (language->texexpr
     #:sets
...
#;(parameterize ([current-rewriters (make-rewriters ~ α)])
  (displayln (texexpr->string (metafunction->texexpr δ_1))))
#;(parameterize ([current-rewriters (make-rewriters ~ α :->)])
  (displayln (texexpr->string (reduction-relation->texexpr ↦))))
